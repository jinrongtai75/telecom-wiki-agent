...
- 관리자만 Gemini API 키 입력 가능 (LLM + 임베딩 공통 키)
...
- API 키는 `GEMINI_API_KEY` 환경변수 → 없으면 `APIKeyManager().get_key("GEMINI")` 순으로 조회
...
- Gemini API 키 없으면 SHA256 해시 기반 폴백 (검색 품질 저하됨)
...
| 키 | 용도 | 설정 주체 |
|----|------|-----------|
| Gemini API 키 | LLM 답변 생성 + 임베딩 | 관리자(antonio)가 DB에 저장, 팀원은 공유 키 사용 |

- 메인 백엔드: 관리자가 설정 패널에서 입력 → DB `gemini_token` 저장 → `_get_llm()` 함수가 DB fallback으로 자동 적용
- 전처리 백엔드: 설정 패널 UI에서 입력 또는 Railway `GEMINI_API_KEY` 환경변수
...
| Table 검수 | `POST /api/objects/{id}/{obj}/table/review` | Gemini로 표 품질 검수 |
| Image 검수 | `POST /api/objects/{id}/{obj}/image/review` | Gemini VLM으로 이미지 설명 |
...
- **한글 파일명**: `Content-Disposition` 헤더는 RFC 5987(`filename*=UTF-8''...`) 형식으로 처리 — latin-1 직접 삽입 시 500 에러
...
| `GEMINI_API_KEY` | Gemini API 키 (유료 플랜 필요) |
