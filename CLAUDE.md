...
[메인 백엔드]        (FastAPI, Railway, PostgreSQL)
      ├─ 적재: md_chunker → Gemini 임베딩 → ChromaDB (Railway)
      │         MD/PDF 원본 → Supabase Storage (영구 보관)
...
| 스토리지 | 위치 | 역할 | 특성 |
|---------|------|------|------|
| **ChromaDB** | Railway 로컬 디스크 | RAG 벡터 검색 (핵심) | Ephemeral — 재시작 시 소실 |
| **Supabase Storage** | Supabase | MD/PDF 원본 백업 | 영구 보관 |
| **PostgreSQL** | Railway | 문서 메타데이터, 사용자, 히스토리 | 영구 보관 |

- **검색 시**: Supabase 무관. ChromaDB에서 벡터 검색 → Gemini LLM 답변
- **Railway 재시작 시**: ChromaDB 소실 → Supabase MD로 자동 재인덱싱 (백그라운드)
- **PDF 미리보기 시**: Supabase에서 원본 PDF 직접 반환
...
client = LLMClient(api_token="...", thinking=False)  # thinking: DB 설정값 전달
response = client.complete(prompt)
```

`thinking` 파라미터는 `search.py`의 `_resolve_thinking(db)`가 DB에서 읽어 전달한다.
`False`(기본) → `thinkingBudget: 0` (빠른 모드), `True` → thinkingConfig 미설정 (사고 모드).
...
인메모리 글로벌 `_thinking_mode` 변수로 모드 관리. `set_thinking_mode(bool)` / `get_thinking_mode()` 사용.
...
**기본값은 빠른 모드** (`thinkingBudget: 0`). 사고 모드는 관리자가 명시적으로 선택해야 한다.

| 항목 | 메인 백엔드 | 전처리 백엔드 |
|------|-----------|------------|
| 저장 방식 | DB `AppSetting("llm_thinking_mode")` | 인메모리 `_thinking_mode` |
| API | `GET/POST /api/settings/llm-mode` | `GET/POST /api/settings/llm-mode` |
| UI | AdminPage (관리자 전용) | SettingsPanel |
| 재시작 후 | DB 유지 | 빠른 모드로 초기화 |
| 적용 범위 | 전체 사용자 공통 | 전체 사용자 공통 |
...
전처리 도구 (PDF/Word 업로드)
  ...
  → 전처리 백엔드 /api/ingest/to-wiki (MD + PDF 전달)
    → wiki agent /api/auth/login (antonio / 비밀번호)
    → wiki agent /api/ingest/md
      → markdowns/{doc_id}.md → Supabase Storage
      → documents/{doc_id}.pdf → Supabase Storage (PDF 첨부 시)
      → md_chunker.chunk_from_text() — obj 주석 파싱 → 섹션별 청크
      → vector_store.index_chunks() — Gemini 임베딩 → ChromaDB (Railway)
```
...
- **반드시 백그라운드 스레드로 실행해야 함** — lifespan 내 동기 실행 시 Gemini API 배치 호출 때문에 Railway healthcheck 타임아웃 발생

재인덱싱 조건: `ChromaDB.count() < DB의 chunk_count 합계`일 때만 실행.

## 참조문서 PDF 미리보기

검색 결과 소스카드의 "미리보기" 버튼은 `has_pdf=true`인 경우에만 표시됨.

- 검색 API에서 `storage.exists(f"documents/{doc_id}.pdf")` 확인 후 `SourceInfo.has_pdf` 설정
- 전처리 에이전트에서 적재 시 **PDF를 함께 전송해야** 미리보기 활성화됨
- PDF 없이 적재된 문서는 미리보기 버튼 자체가 표시되지 않음
- 미리보기를 원하면 전처리 에이전트에서 **PDF 첨부 후 재적재** 필요
...
**속도**: `thinkingBudget: 0` 적용으로 요청당 수초 이내 응답 (이전 30~60초).
**취소**: 프론트엔드에서 `AbortController`로 진행 중 HTTP 요청 즉시 취소 가능. 버튼이 "검수 중 (N/M) 취소"로 변경되며 클릭 시 중단.
**httpx timeout**: 120초 (사고 모드 활성화 시 대비).

검수 결과 → MD 저장:
- Table **keep**: 원본 GFM 표 그대로
- Table **flatten**: Gemini 자연어 텍스트로 대체
- Image **discard**: 객체 삭제 → MD에 미포함
- Image **save**: 설명 텍스트 + `<!-- image: /images/xxx.png -->` 주석
- Image **describe**: Gemini 텍스트 설명만 (이미지 파일 없음)

검수 후 `confirm_status`는 여전히 `pending`. "Confirm" 버튼 필요.
...
`Content-Disposition` 헤더는 RFC 5987(`filename*=UTF-8''...`) 형식으로 처리 — latin-1 직접 삽입 시 500 에러 또는 연결 오류.
...
- **Wiki Agent 비밀번호**: UI 입력 → `.env` 저장 (재배포 시 소실 가능, Railway Variables 권장)
- **LLM 모드**: Segmented 토글 (⚡ 빠른 모드 / 🧠 사고 모드). 기본값 빠른 모드. 재배포 시 초기화.
